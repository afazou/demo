...
class WeixinChat
{
	...
	// 下载媒体文件
    const DOW_MEDIA_URL = 'http://file.api.weixin.qq.com/cgi-bin/media/get?';

    // 上传媒体文件
    const OP_MEDIA_URL = 'http://file.api.weixin.qq.com/cgi-bin/media/upload?';
	...
	/**
	 * 获取接收事件类型
	 * 事件类型如：subscribe(订阅)、unsubscribe(取消订阅)、click、scan(扫描二维码事件已经订阅)
	 */
	 ...
	 */
	public function sendCustomMessage($touser, $data, $msgType = 'text', $errMsg = false)
	{
		$arr = array();
		$arr['touser'] = $touser;
		$arr['msgtype'] = $msgType;
		...
		{
			return $errMsg ? ['errno' => 500, 'msg' => 'token同步失败'] : false;
		}
		...
		if($result)
		{
			...
			if(!$jsonArr || (isset($jsonArr['errcode']) && $jsonArr['errcode'] > 0))
			{
				...
				if(in_array($jsonArr['errcode'],[40001,40014,42001]))
				{
					return $errMsg ? ['errno' => 500, 'msg' => 'token已失效'] : false;
				}
				else
				{
					return $errMsg ? ['errno' => $jsonArr['errcode'], 'msg' => '接口错误|'.$jsonArr['errcode']] : false;
				}
			}
			else
			{
				return $errMsg ? ['errno' => 0, 'msg' => ''] : true;
			}
		}
		
		return $errMsg ? ['errno' => 500, 'msg' => '消息接口请求失败'] : false;
	}
	...
	{
		// 从缓存中获取access_token
		//$cache_flag = 'weixin_access_token_' . $this->token;
        $cache_flag = self::WX_API_TOKEN . $this->appid;
		$access_token = S($cache_flag);
		if($access_token) 
		{
		...
		if($result)
		{
			...
			else
			{
				...
				$expire = isset($jsonArr['expires_in']) ? intval($jsonArr['expires_in'])-1000 : 3600;
				// // 将access_token保存到缓存中
				S($cache_flag, $this->access_token, $expire); 
				return true;
			}
		}
		return false;
	 ...
	 */
	public function redirectGetOauthCode($redirect_uri, $scope=0, $state='',$iurl=1)
	{
	...
	public function UploadMedia($path,$type){
		if(!$this->access_token && !$this->checkAuth()) return false;
		//$path = '@'.APP_ROOT.$path;  //上传图片不添加APP_ROOT变量
		$path = '@'.$path;
		$data = array('media'=>$path);
		$result = curlRequest(self::OP_MEDIA_URL.'access_token='.$this->access_token.'&type='.$type, $data, 'post');
		return $result;
	}

	// 下载多媒体文件
	public function DownloadMedia($media_id,$type='.jpg',$path=''){
		 if(!$this->access_token && !$this->checkAuth()) return false;
         $Http = new \Org\Net\Http();
        if($path=='') {
            $file = C('WEIXIN_FILE_LOCAL') . uniqid() . $type;
        }else{
            $file = $path;
        }
         $Http->curlDownload(self::DOW_MEDIA_URL.'access_token='.$this->access_token.'&media_id='.$media_id,$file);
         return $file;
    }
	...
	public function error($data)
	{
		...
		$Log = new \Think\Log();
		$Log::write('WEIXIN API errcode:['.$this->errCode.'] errmsg:['.$this->errMsg.'] appid:['.$this->appid.'] access_token:['.$this->access_token.']','NOTICE','File');
	}
	...
	{
		if($this->checkSignature()) exit(I('get.echostr'));
	}
