...
class Wechat{
	...
	public function __construct($options)
	{
		...
		$this->redis=RedisCache::get_instance(0);
		//$this->checkAuth();
...
//为了没有appid的订阅号。
	...
	public function DownloadMedia($media_id,$type='.jpg',$path=''){
        ...
        if($path=='')
        {
            $file = WEIXIN_FILE_LOCAL . uniqid() . $type;
        }
        else
        {
            $file = $path;
        }
        $this->curlDownload(self::DOW_MEDIA_URL.'access_token='.$this->access_token.'&media_id='.$media_id,$file);
        return $file;
	}
    ...
    private function curlDownload($remote,$local)
    {
        ...
        @curl_setopt($cp, CURLOPT_CONNECTTIMEOUT, 30);
        $data = curl_exec($cp);
        $status = @curl_getinfo($cp);
        $errno = @curl_errno($cp);
        curl_close($cp);
        fclose($fp);
    }
...
<Nonce><![CDATA[%s]]></Nonce>
	...
	{
		$key=ConfigBase::WX_API_TOKEN.$cachename;
	...
	{
		$key=ConfigBase::WX_API_TOKEN.$cachename;//TODO: get cache implementation
	...
	public function getUserInfo($openid)
	{
		...
		//echo self::API_URL_PREFIX.self::USER_INFO_URL.'access_token='.$this->access_token.'&openid='.$openid;exit;
	 ...
	 * @return string
	 */
	public function getOauthRedirect1314($callback, $state = '', $scope = 'snsapi_userinfo')
	{
		return self::OAUTH_PREFIX.self::OAUTH_AUTHORIZE_URL.'appid=wx1f241d88ae09735c&redirect_uri='.urlencode($callback).'&response_type=code&scope='.$scope.'&state='.$state.'#wechat_redirect';
	}
	 ...
	 * @return array {access_token,expires_in,refresh_token,openid,scope}
	 */
	public function getOauthAccessToken1314()
	{
		$code = isset($_GET['code']) ? $_GET['code'] : '';
		if ( ! $code)
		{
		...
		$result = $this->http_get(self::API_BASE_URL_PREFIX.self::OAUTH_TOKEN_URL.'appid=wx1f241d88ae09735c&secret=7c591bfecf69807aa2393bda68e9f0b6&code='.$code.'&grant_type=authorization_code');
		if ($result)
		{
			$json = json_decode($result, true);
			if ( ! $json ||  ! empty($json['errcode']))
